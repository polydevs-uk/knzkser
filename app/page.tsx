...
// sửa ease -> cubic-bezier array cho đúng type
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-900 via-slate-900/80 to-slate-800 text-slate-100 antialiased">
      <header className="max-w-6xl mx-auto px-6 py-6 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-full bg-gradient-to-br from-indigo-500 to-pink-500 flex items-center justify-center font-bold shadow-2xl">
            NG
          </div>
          <div>
            <div className="text-sm font-medium">KnzkSer</div>
            <div className="text-xs text-slate-400">
              Fullstack • Competitive Programmer • Interaction
            </div>
          </div>
        </div>
      ...
      <main className="max-w-6xl mx-auto px-6 py-12 grid gap-12 lg:grid-cols-2 items-center">
        <section>
          ...
          >
            I am a developer in the field website system and infrastructure
            development. Supporting the leader in deploying AI/ML projects to
            production and optimizing performance.
          </motion.p>
          ...
          >
            ...
            <div className="px-3 py-1 rounded-full bg-slate-800/60">
              Available for hire
            </div>
          </motion.div>
        </section>
            ...
            >
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {p.tags.map((t) => (
                  <span
                    key={t}
                    className="text-xs px-2 py-1 rounded-full bg-slate-700/60"
                  >
                    {t}
                  </span>
                ))}
      ...
      <section id="about" className="max-w-4xl mx-auto px-6 pb-12">
        <motion.div
          ...
          transition={{ duration: 0.6 }}
          className="rounded-2xl p-6 bg-slate-900/60 border border-slate-700"
        >
          <h3 className="text-xl font-semibold">About me</h3>
          <p className="mt-4 text-slate-300">
            Website system and infrastructure development expert. Supporting the
            leader in deploying AI/ML projects to production and optimizing
            performance.
          </p>
      ...
      <section id="contact" className="max-w-4xl mx-auto px-6 pb-20">
        <motion.div
          ...
          whileInView={{ opacity: 1, y: 0 }}
          className="rounded-2xl p-6 bg-gradient-to-tr from-slate-900/60 to-slate-800/60 border border-slate-700"
        >
          <h3 className="text-xl font-semibold">Say hi 👋</h3>
          <p className="mt-2 text-slate-300">
            Want to collaborate? Drop a short message — I will respond within a
            few days. (Demo, Do not use it for now)
          </p>

          <form className="mt-6 grid gap-4 sm:grid-cols-2">
            <input
              className="col-span-2 sm:col-span-1 rounded-xl p-3 bg-slate-800/60 border border-slate-700"
            ...
            />
            <textarea
              className="col-span-2 rounded-xl p-3 bg-slate-800/60 border border-slate-700 min-h-[120px]"
              placeholder="Message..."
            />
            <button className="col-span-2 inline-flex items-center justify-center gap-2 rounded-2xl px-5 py-3 bg-gradient-to-r from-indigo-600 to-pink-600 shadow-md">
              Send
            </button>
          </form>
        </motion.div>
      ...
      <footer className="py-8 text-center text-slate-500 text-sm">
        © {new Date().getFullYear()} KnzkSer — Built with ❤️ & Framer Motion
      </footer>
    </div>
  );
}
...
function FloatingCard({ title, subtitle }: { title: string; subtitle: string }) {
  return (
    <motion.div
      whileHover={{ y: -8 }}
      className="rounded-xl p-4 bg-gradient-to-br from-slate-900/80 to-slate-800/60 border border-slate-700/60 shadow-lg"
    >
      <div className="text-sm font-semibold">{title}</div>
      <div className="text-xs mt-1 text-slate-400">{subtitle}</div>
      <div className="mt-3 text-[10px] px-2 py-1 rounded-full bg-slate-700/50 inline-block">
        Interactive
      </div>
    </motion.div>
  );
  ...
  return (
    <div>
      ...
      <div className="mt-2 w-full bg-slate-800/50 rounded-full h-2 overflow-hidden">
        <motion.div
          initial={{ width: 0 }}
          whileInView={{ width: `${Math.round(level * 100)}%` }}
          transition={{ duration: 0.8 }}
